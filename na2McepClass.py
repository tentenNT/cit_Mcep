...
def readfiles(data_location):
    for i in range(1, 101):
        if i < 10:
            yield Path(data_location+"/" + data_location + "_00{}.txt".format(i)).read_text()
        elif i < 100:
            yield Path(data_location+"/" + data_location + "_0{}.txt".format(i)).read_text()
        else:
            yield Path(data_location+"/" + data_location + "_{}.txt".format(i)).read_text()
